...
int main(int argc, char **argv)
{
    ...
    /*cons.one = conduct_create("test", sizeof(struct julia_request),
                              QSIZE * sizeof(struct julia_request));*/
	cons.one = conduct_open("test");
    if(cons.one == NULL) {
        perror("conduct_create");
        exit(1);
    }
/*
    cons.two = conduct_create("tost", sizeof(struct julia_reply),
                              QSIZE * sizeof(struct julia_reply));*/
	cons.two = conduct_open("tost");
    if(cons.two == NULL) {
        perror("conduct_create");
        exit(1);
    }
    ...
    for(int i = 0; i < numthreads; i++) {
		perror("je rentre dans la boucle principale");
        pthread_t t;
    ...
    gtk_main();
	conduct_close(cons.one);
	conduct_close(cons.two);
	//conduct_destroy(cons.one);
	//conduct_destroy(cons.two);
    return 0;
}
